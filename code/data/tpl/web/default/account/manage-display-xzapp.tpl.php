...
<div class="clearfix we7-margin-bottom">
	<?php  if(!$_W['isfounder'] && !empty($account_info['xzapp_limit']) || user_is_vice_founder()) { ?>
	<div class="alert alert-warning hidden">
		...
		可创建 <span class="text-strong"><?php  echo $account_info['maxxzapp'];?> </span>个熊掌号，已创建<span class="text-strong"> <?php  echo $account_info['xzapp_num'];?> </span>个，还可创建 <span class="text-strong"><?php  echo $account_info['xzapp_limit'];?> </span>个熊掌号。
	</div>
	<?php  } ?>
	<form action="" class="form-inline  pull-left" method="get">
		<input type="hidden" name="c" value="account">
		<input type="hidden" name="a" value="manage">
		<input type="hidden" name="account_type" value="<?php echo ACCOUNT_TYPE_XZAPP_NORMAL;?>">
