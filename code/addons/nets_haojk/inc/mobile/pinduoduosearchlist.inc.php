...
getUserInfo(0);
get_global();
//查询当前会员 原openid变更为uid zxq.2018.05.17 in pinduoduosearchlist.inc.php in 8
...
$cname=$_GPC["cname"];
if(!empty($cname)){
	$goodsource=pdo_fetch("SELECT * from ".tablename("nets_hjk_pdd_menu")." where id=:id and uniacid=:uniacid",array(":id"=>$cname,":uniacid"=>$_W["uniacid"]));
	if(!empty($goodsource)){
		//自定义商品原的跳转到专题页面
		$url=$this->createMobileUrl('pinduoduoclasslist',array("sourceid"=>$_GPC["cname"]));  
        Header("Location: $url");  
	}
}
foreach($level AS $l){
...
if(!empty($_GPC["ajax"])){
	...
	* page 当前页数 默认1，必选
	* pagesize 每页记录数 默认20
	* sort 排序 默认0，券后价1，优惠幅度2，佣金比例3，销量4(暂时无用),6两小时跑单，8实时人气榜，传6、8的时候其他可以不传
...
include $this->template('searchlist');
